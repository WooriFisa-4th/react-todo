...
const TodoItem = ({todo}) => {
  ...
  const {currentCategory} = useTodoContext();
  const isFiltered = (currentCategory !== 'all');

  if(isFiltered && (category !== currentCategory)) return (<></>);
  ...
  return (
    <li className="flex gap-4 justify-between my-4 py-4 px-4 border-[1px] bg-gray-700 rounded-md shadow-xl">
        ...
        <div className="flex items-center gap-1">
            ...
            <IconButton onClick={() => dispatch({type: 'DELETE', id: id})} icon={'🗑'} />
        </div>
        {modal && createPortal(
          <Modal>
            <TodoForm onClose={() => setModal(false)} todo={todo}>
              할일 수정
            </TodoForm>
          </Modal>,
          document.body
